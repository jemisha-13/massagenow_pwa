...
export class ToasterService {
  ...
  success(message: string, keepAfterNavigationChange = false, timer = 2000) {
  ...
  error(message: string, keepAfterNavigationChange = false, timer = 2000) {

    this.keepAfterNavigationChange = keepAfterNavigationChange;
    this.subject.next({ type: 'error', text: message, timer, msgId: this.genrateId() });
  }

  info(message: string, keepAfterNavigationChange = false, timer = 2000) {

    this.keepAfterNavigationChange = keepAfterNavigationChange;
    this.subject.next({ type: 'info', text: message, timer, msgId: this.genrateId() });
  }

  warning(message: string, keepAfterNavigationChange = false, timer = 2000) {
